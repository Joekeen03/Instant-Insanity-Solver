...
def DepthFirstFinder(puzzle, maxLength, startDepth=0, logFile=None):
    ...
    def ConditionalOutput(depth, sliceID):
        print(f"Searching @ {depth}/{limit} (depth/limit), slice ID {sliceID}", file=logFile, flush=True)
    ...
    def DFRecursiveFast(prevSolutions, depth, startIndex):
        ...
        while (j < (puzzleLength-limit+depth)) and (limit >= depth):
            ...
            if len(currSolutions) > 0: # This particular subset has a solution
                ...
                limit = len(smallestObstacle)-1-1
                print(f"Minimal obstacle size reduced to {limit+1}, while processing subset of size {depth+1}", file=logFile, flush=True)
                return
            j += 1
    ...
    def DFRecursive(prevSolutions, depth, startIndex):
        ...
        while j < (puzzleLength-limit+depth):
            ...
            if len(currSolutions) > 0: # This particular subset has a solution
                ...
                temp = SF.FindObstacleReductive(subPuzzle)
                smallestObstacle = list(map(lambda i: subPuzzle.mappingIndices[i], temp.mappingIndices))
                print(smallestObstacle, file=logFile, flush=True)
                limit = len(smallestObstacle)-1-1
                print(f"Minimal obstacle size reduced to {limit+1}, while processing subset of size {depth+1}", file=logFile, flush=True)
                return
            j += 1
    ...
    while (j < (puzzleLength-limit+startDepth)) and (limit > startDepth):
        ...
        f = ConditionalOutput(startDepth, j)
        indices[startDepth] = j
        setSize = limit-startDepth
        subSetSize = math.factorial(puzzleLength-j-1)/(math.factorial((puzzleLength-j-1) - setSize)*math.factorial(setSize))
        ...
        result = DFRecursive(sideColors, startDepth+1, j+1)
        j += 1
...
def FindSmallestObstacle(puzzle, logFile=None):
    ...
    if logFile != None:
        print(f"Processing puzzle:\n{puzzle}")
    print("Running Separation Solver.", file=logFile, flush=True)
    result = SS.SeparationSolver(puzzle)
    if result.isFullSolution:
        print("Puzzle has solution. Terminating.", file=logFile, flush=True)
        return None

    smallestObstacle = None
    print("Puzzle has no solution. Checking independent subpuzzles", file=logFile, flush=True)
    for subResult in result.subResults:
        if not subResult.isFullSolution:
            print("Found a subpuzzle without a solution.", file=logFile, flush=True)
            subPuzzle = subResult.puzzle
            print(f"Searching subpuzzle:\n{subPuzzle}", file=logFile, flush=True)
            firstObst = SF.FindObstacleReductive(subPuzzle)
            print(f"Found a minimal obstacle:\n{firstObst}", file=logFile, flush=True)
            # Find slices which must be in all minima obstacles - this can dramatically cut down the search space
            ...
            sizeBound = min(len(obst), len(smallestObstacle))
            print("Searching for smallest minimal obstacle smaller than sizeBound.", file=logFile, flush=True)
            currObst = DepthFirstFinder(optimizedSubPuzzle, sizeBound, len(required), logFile)
            if len(currObst) != 0:
                print(f"Found smallest minimal obstacle so far:\n{currObst}")
                smallestObstacle = currObst
        else:
            print("Found a subpuzzle with a solution. Skipping:\n{subResult}", file=logFile, flush=True)
